...
llm = OpenAI(model="gpt-3.5-turbo", api_base='http://gemini2.sufe.edu.cn:27282/v1', api_key='null', timeout=300)
embed_model = OpenAIEmbedding(api_base='http://gemini2.sufe.edu.cn:27282/v1', api_key='null', timeout=300)
...
kg_nodes_dc = {}
for book_name, document in tqdm(zip(file_name_ls, document_ls), total=len(file_name_ls)):
    nodes_group = book_name.replace('.pdf', '')
    splitter = SentenceSplitter(chunk_size=chunk_size, chunk_overlap=chunk_overlap)
    split_document = splitter.get_nodes_from_documents(documents=document)
    for node in split_document:
        node.text = node.text.replace(useless_text[0], '').replace(useless_text[1], '').replace(useless_text[2], '')
    kg_nodes_dc[nodes_group] = split_document
...
for book_name, document in tqdm(zip(file_name_ls, document_ls), total=len(file_name_ls)):
    ...
    for node in split_document:
        node.text = node.text.replace(useless_text[0], '').replace(useless_text[1], '').replace(useless_text[2], '')
    vec_nodes_dc[nodes_group] = split_document
...
for nodes_group, nodes in kg_nodes_dc.items():
    thread = Thread(target=create_and_store_kg_index, args=(nodes_group, nodes))
    thread.start()

for nodes_group, nodes in vec_nodes_dc.items():
