...
class ModelClient(Protocol):
    ...
    RESPONSE_USAGE_KEYS = ["prompt_tokens",
                           "completion_tokens", "total_tokens", "cost", "model"]
    ...
    def create(self, params: Dict[str, Any]) -> ChatCompletion:
        ...
        if params.get("stream", False) and "messages" in params:
            ...
            model = chunk.model.replace(
                "gpt-35", "gpt-4")  # hack for Azure API
            model = chunk.model.replace(
                "chatglm3-6b", "gpt-4")  # hack for chatglm3-6b
...
class OpenAIWrapper:
        ...
        if config_list:
            ...
            for config in config_list:
                self._register_default_client(
                    config, openai_config)  # could modify the config
        ...
        for i, client in enumerate(self._clients):
            if isinstance(client, PlaceHolderClient):
                ...
                if placeholder_config.get("model_client_cls") == model_client_cls.__name__:
                    self._clients[i] = model_client_cls(
                        placeholder_config, **kwargs)
                    return
    ...
    def create(self, **config: Any) -> ModelClient.ModelClientResponseProtocol:
        ...
        for i, client in enumerate(self._clients):
            ...
            cache_seed = extra_kwargs.get(
                "cache_seed", LEGACY_DEFAULT_CACHE_SEED)
            ...
            if cache_client is not None:
                with cache_client as cache:
                    ...
                    response: ModelClient.ModelClientResponseProtocol = cache.get(
                        key, None)
                    ...
                    if response is not None:
                        ...
                        if pass_filter or i == last:
                            ...
                            self._update_usage(
                                actual_usage=actual_usage, total_usage=total_usage)
                            return response
            ...
            try:
                ...
                self._update_usage(actual_usage=actual_usage,
    ...
    def _update_usage(self, actual_usage, total_usage):
        ...
        if total_usage is not None:
            self.total_usage_summary = update_usage(
                self.total_usage_summary, total_usage)
        if actual_usage is not None:
            self.actual_usage_summary = update_usage(
                self.actual_usage_summary, actual_usage)
